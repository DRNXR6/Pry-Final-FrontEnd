...
function ContMarketPlace() {
    ...
    function FunctionDetails(usuario, id) {
        setTimeout(() => {
            ...
            localStorage.setItem("UserOwner", JSON.stringify(usuario));            
                    ...
                    {filteredConsultas.map((publication, index) => (
                        <article key={index}>
                            <div className="ItemCard">
                                <img onClick={() => FunctionDetails(publication.usuario, publication.id)} className="imgCard" src={publication.imgName} alt="" />
